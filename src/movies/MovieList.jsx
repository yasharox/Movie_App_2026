...
//         open={open}
//         onClose={handleClose}
//         disableEnforceFocus={true} // ✅ Prevents the browser focus loop collision
//         disableAutoFocus={true} // ✅ Stops the dialog from fighting the button for focus
...
function MovieList() {
  ...
  return (
    <section className="movie-list">
      {/* 1. FIX: Destructure both _id (MongoDB) and id (fallback local data) */}
      {movie.map(({ name, image, rating, summary, _id, id }) => {
        // 2. FIX: Pick whichever identifier exists
        const actualId = _id || id;

        return (
          <Movie
            key={actualId} // 3. FIX: Clears the 'unique key' console warning completely
            name={name}
            poster={image}
            rating={rating}
            summary={summary}
            ...
            editButton={
              <IconButton
                ...
                size="large"
                onClick={() => {
                  history.push("/movies/edit/" + actualId); // 4. FIX: Use working ID string
                }}
              >
                <EditIcon />
              </IconButton>
            ...
            deleteButton={
              <IconButton
                ...
                size="large"
                onClick={() => {
                  handleDeleteClick(actualId); // 5. FIX: Passes the true database key string
                }}
              >
                <DeleteIcon />
              </IconButton>
